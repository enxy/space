	...
	*/
	const defaults = {
		...
		controlText: {
			...
			monthChange: 'Left click to scrolls months, right to change month'
		},
		runningServer: {
			host: 'localhost',
			port: 3000,
	...
	const classes = {
		...
		currentMonth: 'jqTimelineCurrentMonth',
	};
	...
	Timespace = {
		...
		monthTableHead: '<header id="monthTable" style="left: 0px;"></header>',
		timeTableBody: '<section></section>',
		display: '<article class="jqTimespaceDisplay" style="margin: auto;">\
					<span id="year_prev"> << 2018</span>\
					<span id="year_cur"> 2019 </span>\
					<span id="year_next"> >> 2020 </span>\
				</article>',
		displayWrapper: '<div></div>',
		displayTitle: '<div></div>',
		displayTimeDiv: '<div class="jqTimespaceDisplayTime"></div>',
		yearBody: '<span id="year">' + new Date().getFullYear() + '</span>',
		displayTime: '<time></time>',
		displayBody: '<section></section>',
		absenceLegend: '<div><ul>',
		...
		load: function (target, options, id) {
			...
			this.data = opts.data || {};
			this.totalTime = ((opts.endTime-1) - opts.startTime) || 1;
			this.navInterval = {
				dir: 'left',
				timer: null,
				engaged: false,
			...
			//this.requestAbsences(this.options.selectedMonth);
			...
			this.yearLeft = $(this.navLeft)
				.attr('title', opts.controlText.navLeft)
				.appendTo(this.yearRow);
			...
			this.navRight = $(this.navRight)
				...
				.appendTo(this.dataContainer);
			this.yearBody = $(this.yearBody).appendTo(this.display);
			...
			this.monthView = {
				...
				offsetY: 0,
				offsetX: 0,
				monthTableHeadOffsetX: 0,
				monthTableHeadOffsetY: 0,
				shiftOriginX: 0,
				shiftOriginY: 0,
				tableWidth: 0,
				totalTime: 0,
			...
			// add event to scroll throw months
			this.monthTableHead.on('mousedown', this.moveMonthsHeader.bind(this));
			
			// mark current month
			document.querySelectorAll('h1')[opts.selectedMonth-1].classList.add('jqSelectedMonth');
		...
		moveMonthsHeader: function(e){
			if (e.which == 1) {
				if (this.monthTableHead.length) {
					let posX = -parseInt(this.monthTableHead[0].style.left.match(/\d+/)[0]);
					let move = parseInt(this.options.monthTableMove);
					let w = this.viewData.tableWidth;

					if (this.viewData.halfX < e.pageX) {
						posX -= this.options.monthTableMove;
					} else {
						posX += this.options.monthTableMove;
					}

					if (0 >= posX && Math.abs(posX) < this.viewData.tableOffsetX) {
						this.monthTableHead[0].style.left = posX + "px";
					}
				}
			}
		},
		...
		setupEvents: function() {
			...
			let ids = {"prev": current-1 , "current": current , 'next': current+1};

			document.querySelectorAll('.jqYearSpan').forEach(span => {
				span.innerText = ids[span.id];
			});

			// get absences after clicking month heading
			for (let i=0; i<12; i++) {
				this.wideHeadings[i].addEventListener('mousedown', this.changeMonth.bind(this, i), false);
			}
		...
		buildTimeTable: function () {
			...
			this.monthView.tableWidth = opts.markerAmount * opts.markerWidth || 'auto';
			this.monthTableHead = $(this.monthTableHead)
				.attr('title', opts.controlText.monthChange)
				.width(this.monthView.tableWidth)
				.appendTo(this.dataContainer);
		...
		/**
		 * Select first most actual absence from timeline.
		 * If other then current month is selected, chose
		 * first absence of selected month.
		 */
		selectFirstAbsence: function() {
			let t = parseInt(new Date().getDate()+10);
			let i=0;
			
			if (this.options.selectedMonth == (new Date().getMonth()+1)) {
				while (true) {
					let v = parseInt(this.data.events[i].start.split('.')[0]);
					if (t<=v) {
						this.timeEvents.eq(i).trigger('mouseup', [true]);
						break;
					}
					i++;
				}
			} else {
				this.timeEvents.eq(0).trigger('mouseup', [true]);
			}
		...
		buildTimeHeadings: function () {
			...
			let h1 = `<h1><span class="${classes.heading}"></span></h1>`,
				...
				headings = $('<div></div>'),
				yearHeading = $('<div class="jqTimespaceDisplay"></div>'),
				curSpan = 0;
			...
			if (headings.length > 0) {
				...
				headings.appendTo(this.monthTableHead);
				//this.titleClamp.css('top', headings.innerHeight() / 2);
			}

			yearHeading.appendTo(this.timeTableHead);

			for (let i=0; i<headings[0].children.length; i++) {
				if (i == new Date().getMonth()) {
					headings[0].children.item(i).classList.add('jqTimelineCurrentMonth');
				}
			}
		...
		/**
		 * 
		 * @param {*} month selected month from timeline
		 * @param {*} e click event
		 */
		changeMonth: function(month, e) {
			if (e.which == 3) {
				this.requestAbsences(month);
				document.querySelectorAll('.jqSelectedMonth').forEach(node=>{
					node.classList.remove('jqSelectedMonth');
				});
				document.querySelectorAll('h1')[month].classList.add('jqSelectedMonth');
			}
		},
		 ...
		 * @return {Object} The Plugin instance
		 */
		requestAbsences: function(month) {
			let i = month + 1;
			let year = this.options.selectedYear;
			let id = i +'-' + year;			
			const server = this.options.runningServer;
			var params = '/' + i + '/' + year;
			...
			var event = new CustomEvent("storageChanged", { "detail": id});

			if (localStorage.getItem(id)) {
				this.data.events = JSON.parse(localStorage.getItem(id));
			} else {
				...
				xhr.onload = function () {
					if (xhr.readyState === xhr.DONE) {
						if (xhr.status === 200) {
							...
							this.emptyTimeline();
							//document.querySelector('.jqTimepsaceContainer aside section').style.minHeight = "334px";
						}
					}
				};
			...
			// listen if storage has changed
			document.addEventListener('storageChanged', function(e) {
				// prepare container for day events in month	
				let day_events = {};
				for (let i=1; i<32; i++)
					day_events[i] = [];
				//selected month
				let monthEvents = JSON.parse(localStorage.getItem(e.detail));
				this.data.events = monthEvents;
				// if (monthEvents < 10) {
				// 	document.querySelector('.jqTimepsaceContainer aside section').style.minHeight = "334px";
				// }
			}.bind(this), false);

			let curNumDays = this.getNumberOfDaysInMonth(parseInt(month)+1, year);
			let prevNumDays = this.getNumberOfDaysInMonth(this.options.selectedMonth, this.options.selectedYear);

			let diff = curNumDays-prevNumDays; 
			if (diff<0)
				this.removeColumns(diff);

			if (diff>0)
				this.addColumns(diff);
			this.options.selectedYear = year;
			this.changeTimeLabels(month+1, year);
			this.markMonthWeekends(month+1, year);
			this.buildTimeEvents().buildTimeDisplay()
				.updateStaticData()
				.updateDynamicData()
				.setDOMEvents()
				.selectFirstAbsence();

			this.options.selectedMonth = parseInt(month) + 1;
			
			return this;
		},

		removeColumns: function(num) {
			let columns = document.querySelectorAll('.jqTimespaceColumn');
			let parentColumn = document.querySelector('section');
			let timeHeaders = document.querySelectorAll('time');
			let parentHeader = timeHeaders[0].parentNode;
			num = Math.abs(num);

			for (let i=0; i<num; i++) {
				parentColumn.removeChild(parentColumn.lastChild);
				parentHeader.removeChild(parentHeader.lastChild);
				this.markers.splice(-1,1);
			}

			let widthDiff = num*3 + parseInt(timeHeaders[0].style.width.match(/\d+/)[0]);
			this.options.markerWidth = widthDiff;
			for (let i=0; i<columns.length; i++) {
				timeHeaders[i].style.width = widthDiff.toString() + "px";
				columns[i].style.width = widthDiff.toString() + "px";
			}
		...
		addColumns: function(num) {
			...
			let newWidth = this.options.markerWidth - num * 3;
			let  i = 0;
			timeHeaders.forEach(time=>{
				time.style.width = newWidth.toString() + "px";
				if (i<columns.length)
					columns[i].style.width = newWidth.toString() + "px";
				i++;
			});
			this.options.markerWidth = newWidth;

			// build new columns (months can differ by 1-2 days)
			for (let i=0; i<num; i++) {
				let column = document.createElement("div");
				let time = document.createElement("time");
				// new column
				column.classList.add("jqTimespaceColumn");
				column.style.width ="100px";
				// date date
				time.style.width = "100px";
				let m = parseInt(this.options.selectedMonth);
				m = m < 10 ? "0" + m : m;
				time.innerText = timeHeaders.length.toString() + "." + m;
				// append to timeline
				document.querySelector('section').appendChild(column);
				timeHeaders[0].parentNode.appendChild(time);
			}
			return this;
		},

		/**
		 * Empty timeline from prev month's absences
		 * 
		 */
		emptyTimeline: function() {
			document.querySelectorAll('.jqTimespaceColumn').forEach(column=>{
		...
		getWeekdaysInMonth: function(month, year) {
			  var stop = this.getNumberOfDaysInMonth(month, year);
			  ...
			  for(var i=1; i<=stop; i++) {
				  ...
				  if (this.isWeekday(year, month, i)) { 
					  var y = wday.getFullYear(), 
					  m = (wday.getMonth()+1 < 10) ? "0"+(wday.getMonth()+1) : wday.getMonth()+1,
		...
		changeTimeLabels: function(month, year) {
			let timeHeaders = document.querySelectorAll('time');
			month = (parseInt(month) < 10) ? "0" + month : month;
			let day = 1;
			timeHeaders.forEach(header=>{
				header.innerText = day + "." + month;
				day++;
			});
		},

		markMonthWeekends: function(month, year) {
			var weekdays = this.getWeekdaysInMonth(month, year);
			let timeHeaders = document.querySelectorAll('time');
			let columns = document.querySelectorAll('.jqTimespaceColumn');
			...
			timeHeaders.forEach(time=>{
				let columns = document.querySelectorAll('.jqTimespaceColumn');
					let date =  time.innerText.split('.');
					let day = (parseInt(date[0])<10) ? "0" + date[0] : date[0];
					var rDate = year + '-' + [date[1], day].join('-');
					if (weekdays.indexOf(rDate) == -1){
						time.style.backgroundColor = "#494e58";
						if (columns[i]) {
							columns[i].style.backgroundColor = "#494e58";
							columns[i].style.opacity = "0.3";
						}
					}
					i++;
			});
		...
		clearWeekendMarks: function() {
			let timeHeaders = document.querySelectorAll('time');
			let columns = document.querySelectorAll('.jqTimespaceColumn'), i = 0;

			timeHeaders.forEach(time=>{
				time.style.backgroundColor = "";
				if (columns[i]) {
					columns[i].style.backgroundColor = "#282c34";
		...
		buildTimeEvents: function () {
			...
			if (this.data.events) {
				this.data.events.forEach((v, i) => {
					...
					rowData.eventElem = $(`<p${evtClass} type="` + v.type + `"><span>${title}</span></p>`).prependTo(rowData.event);
					...
					if (index >= 0) {
						...
						rowData.eventElem.width(eventElemWidth)
							...
							.attr('title', rowData.eventElem.data('time')+ '.' + this.options.selectedYear);
				...
				// set absence colors according type
				document.querySelectorAll('.jqTimespaceEvent p ').forEach(
					event=>{
						let type = event.getAttribute('type');
						let color = this.options.absenceTypes[type];
		...
		buildTimeDisplay: function () {
			...
			this.displayTimeDiv = $(this.displayTimeDiv).appendTo(this.displayWrapper).css('width', "100%;");;
			...
			this.displayRight = $(this.displayRight)
				...
				.appendTo(this.displayTimeDiv);
			this.displayTitle = $(this.displayTitle).appendTo(this.displayBody);
			...
			Object.keys(opts.absenceTypes).forEach(type=>{
				this.absenceLegend += '<li style="background-color:' + opts.absenceTypes[type] +';">' + type + '</li>';
			});
			this.absenceLegend += '</ul></div>';
			this.absenceLegend = $(this.absenceLegend).appendTo(this.displayWrapper);
		...
		setDOMEvents: function () {
			...
			// Event Marker Events - must be animated on each calendar dynamic modification!
			...
			// timeline container is not chaneged, only events and headers
			// assure flag is set to true to avoid animation problems.
		...
		timeShiftCache: function (plane, val, finished) {
			...
			if (val !== this[lastMousePos] && !finished) {
				...
				this[shiftDiff] = dir;
				this.monthView.shiftDiff = dir;
				this[shiftPos] = this[posMethod]() + (dir * this.options[dragMultiplier]);
				this.monthView.shiftPos = this[posMethod]() + (dir * this.options[dragMultiplier]);
				this[shiftDir] = (dir < 0) ? '<' : '>';
				this.monthView.shiftDir = (dir < 0) ? '<' : '>';
				this[lastMousePos] = val;
				this.monthView.lastMousePos = val;
		...
		getMonthTableHead: function (offset) {
			return parseFloat((offset) ? this.timeTable.offset().left : this.timeTable.css('left'));
		},
		...
		updateDynamicData: function () {
			...
			this.displayTimeDiv.css('marginLeft', this.viewData.halfX-80);
		...
		changeYear: function(type) {
			...
			let text = (type == 'prev') ? curYear-1 : curYear+1 ;
			this.options.selectedYear = text;
			document.getElementById('year').innerHTML = text;
			this.requestAbsences(1);
		},
